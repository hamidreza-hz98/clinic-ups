    ...
    <Box ref={rootRef} sx={{ overflow: "hidden", bgcolor: "background.default", color: "text.primary" }}>
      ...
      <Box component="section" sx={{ position: "relative", py: { xs: 9, md: 14 }, bgcolor: "background.paper", borderTop: "1px solid rgba(var(--landing-secondary-rgb),.08)" }}>
        ...
        <Container maxWidth="xl" sx={{ position: "relative", zIndex: 1 }}>
          ...
          <Stack direction={{ xs: "column", md: "row" }} justifyContent="center" spacing={2} className="contact-page-reveal" sx={{ mt: 4 }}>
            {socialMediaItems.map((social) => <LiquidGlass key={social.id} component="a" href={social.address} target="_blank" rel="noreferrer" interactive intensity="subtle" sx={{ px: 2.5, py: 1.5, borderRadius: 99, color: "inherit", display: "flex", alignItems: "center", gap: 1.2 }}><SocialIcon network={social.id} style={{ width: 28, height: 28 }} as="span" /><Typography sx={{ fontSize: ".8rem" }}>{faText(social.name)}</Typography></LiquidGlass>)}
          </Stack>
        </Container>
      </Box>
      ...
      <Box component="section" id="contact-form" sx={{ position: "relative", py: { xs: 9, md: 14 }, bgcolor: "background.default", scrollMarginTop: 100 }}>
        <Container maxWidth="xl">
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "5fr 7fr" }, gap: { xs: 5, lg: 6 }, alignItems: "stretch" }}>
            ...
            <SpotlightGlass component="form" onSubmit={submit} intensity="strong" className="contact-page-reveal" sx={{ p: { xs: 3, sm: 4, md: 5 }, borderRadius: 6 }}>
              <Box sx={{ display: "contents" }}>
                ...
                <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, minmax(0, 1fr))" }, gap: 2.2 }}>
                  <TextField required fullWidth label="نام و نام خانوادگی" value={form.fullName} onChange={updateField("fullName")} autoComplete="name" slotProps={{ input: { startAdornment: <InputAdornment position="start"><PersonOutlineRoundedIcon color="primary" /></InputAdornment> } }} />
                  <TextField required fullWidth label="شماره موبایل" value={form.mobile} onChange={updateField("mobile")} type="tel" inputMode="tel" autoComplete="tel" slotProps={{ input: { startAdornment: <InputAdornment position="start"><SmartphoneRoundedIcon color="primary" /></InputAdornment> } }} />
                  <TextField required fullWidth multiline minRows={6} label="پیام شما" value={form.message} onChange={updateField("message")} sx={{ gridColumn: { sm: "span 2" } }} slotProps={{ input: { startAdornment: <InputAdornment position="start" sx={{ alignSelf: "flex-start", mt: 1.5 }}><AlternateEmailRoundedIcon color="primary" /></InputAdornment> } }} />
                </Box>
