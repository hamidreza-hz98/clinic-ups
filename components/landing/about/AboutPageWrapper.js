  ...
  return (
    <Box ref={rootRef} sx={{ overflow: "hidden", bgcolor: "background.default", color: "text.primary" }}>
      <Box component="section" sx={{ position: "relative", isolation: "isolate", minHeight: { xs: 790, md: 850 }, pt: { xs: 15, md: 17 }, pb: { xs: 9, md: 12 }, display: "flex", alignItems: "center" }}>
        ...
        <Box aria-hidden sx={{ position: "absolute", inset: 0, zIndex: 0, background: "radial-gradient(circle at 22% 44%, rgba(var(--landing-accent-rgb),.13), transparent 30%), linear-gradient(180deg, transparent 50%, var(--landing-bg) 100%)" }} />

        <Container maxWidth="xl" sx={{ position: "relative", zIndex: 1 }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "7fr 5fr" }, gap: { xs: 6, md: 8 }, alignItems: "center" }}>
            <Box>
              <Chip icon={<HubRoundedIcon />} label="22 YEARS OF CONTINUOUS ENERGY" variant="outlined" className="hero-reveal hero-reveal-1" sx={{ mb: 3, direction: "ltr", color: "primary.light", borderColor: "rgba(var(--landing-accent-rgb),.28)", bgcolor: "rgba(var(--landing-accent-rgb),.055)", "& .MuiChip-icon": { color: "primary.main" } }} />
            ...
            <Stack spacing={2.5}>
              {values.map(({ icon: Icon, title, description }, index) => (
                <SpotlightGlass key={title} intensity="medium" interactive className="about-page-reveal" sx={{ "--about-reveal-delay": `${index * 90}ms`, p: { xs: 3, md: 4 }, borderRadius: 5 }}>
                  <Stack direction="row" spacing={2.5} alignItems="flex-start"><Box sx={{ flex: "0 0 auto", width: 54, height: 54, display: "grid", placeItems: "center", borderRadius: 3, color: "primary.main", bgcolor: "rgba(var(--landing-accent-rgb),.08)", border: "1px solid rgba(var(--landing-accent-rgb),.18)" }}><Icon sx={{ fontSize: 28 }} /></Box><Box><Typography component="h3" variant="h5" sx={{ mb: 1 }}>{title}</Typography><Typography color="text.secondary" sx={{ lineHeight: 2 }}>{description}</Typography></Box></Stack>
                </SpotlightGlass>
              ))}
            </Stack>
          </Box>
      ...
      <Box component="section" sx={{ position: "relative", py: { xs: 9, md: 14 }, bgcolor: "background.default" }}>
        <Container maxWidth="xl">
          <Box className="about-page-reveal" sx={{ textAlign: "center", mb: { xs: 6, md: 8 } }}><Chip icon={<TimelineRoundedIcon />} label="روش کار ما" variant="outlined" sx={{ mb: 2, color: "primary.light", borderColor: "rgba(var(--landing-accent-rgb),.25)" }} /><Typography component="h2" variant="h2" sx={{ fontSize: { xs: "2.4rem", md: "3.6rem" }, mb: 1.5 }}>از مسئله تا پایداری</Typography><Typography color="text.secondary">سه مرحله متصل، یک مسئولیت مشترک</Typography></Box>
