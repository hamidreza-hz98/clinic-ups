...
const ProductForm = ({ data, mode = "create", onSubmit }) => {
  ...
  React.useEffect(() => {
    ...
    if (data.category) setValue("category", data.category._id || data.category);

    if (data.brand) setValue("brand", data.brand._id || data.brand);
  ...
  }, [categories, products, brands, tags, data, setValue]);
            ...
            <Controller
              ...
              render={({ field }) => {
                const selectedCategory =
                  categories?.find(
                    (c) => c._id === (field.value?._id || field.value),
                  ) || null;
              ...
              render={({ field }) => {
                ...
                return (
                  <Autocomplete
                    ...
                    getOptionLabel={(option) => option?.name || ""}
                    value={selectedBrand}
                    noOptionsText="برندی ای یافت نشد!"
                    onChange={(e, newValue) =>
                      field.onChange(newValue?._id || null)
                    }
                    renderInput={(params) => (
